...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        try:
            ...
            dir_paths = self.handle_dirs(database, ["updates", "index", "signatures", "failed", "lists", "manifests"])
            self.create_initial_manifests(manifest, dir_paths)
            LOGGER.info(f"Creating manifests finished.")
    ...
    def create_initial_manifests(self, manifest, dir_paths):
        ...
        if not manifest_IDs and index_paths:
            ...
            for i,IDs in manifest_dict.items():
                self.save_pickle(IDs, os.path.join(dir_paths["manifests"], f"wort-sra-kmer-db{i}.pcl"))
            LOGGER.info(f"Finished, creating new starting manifest {len(manifest_dict)} for further updates.")
            self.save_pickle([], os.path.join(dir_paths["manifests"], f"wort-sra-kmer-db{len(manifest_dict)}.pcl"))
