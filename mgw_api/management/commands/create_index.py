...
class Command(BaseCommand):
        ...
        try:
            ...
            if new_sig_files:
                ...
                for i in range(0, len(sig_files), settings.INDEX_MAX_SIGNATURES):
                    ...
                    results = [self.update_index(dir_paths, sig_list, database, k, last_num+i) for k in kmers]
                    td, rs = ("failed", ".err") if sum([int(r.returncode) for r in results]) else ("signatures", "")
                    self.move_files(new_files, dir_paths, td, rs)
                    self.update_manifests(new_files, mani_list, manifest, dir_paths, last_num+i)
                    LOGGER.info(f"Updated index # {last_num+i} with {len(last_sig_files)} recalculated signatures and {len(new_sig_files)} new added signatures.")
                self.save_pickle(set(), os.path.join(settings.DATA_DIR, database, "metagenomes", f"update_successful.pcl"))
    ...
    def get_last_index(self, dir_paths):
        ...
        last_sig_files = [os.path.join(dir_paths["signatures"], f"{ID}.sig.gz") for ID in last_sig_IDs]
    ...
    def update_index(self, dir_paths, sig_list, database, k, last_num):
        idx = os.path.join(dir_paths["index"], f"wort-{database}-{k}-db{last_num}.rocksdb")
